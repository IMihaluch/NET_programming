...
Fileloader::Fileloader(const RemoteResource& remote_res):
...
const cpr::Url Fileloader::createURL() const {
    return "https://" + res_.host + ":" + res_.port + res_.target;
}

const std::shared_ptr<cpr::Response> Fileloader::fetchRemoteStuff() const {
...
const std::string Fileloader::getFetchedHeader() const {
    if(hasSuccess())
...
const std::string Fileloader::getFetchedPayload() const {
...
void Fileloader::printServerCertificate() const {
    if(!hasSuccess())
        return;
...
const bool Fileloader::saveFetchedPayload() const {
    if(!hasSuccess())
        return false;
...
const bool Fileloader::hasSuccess() const {
    return stuff_received_ != nullptr;
}

const bool Fileloader::is200OK() const {
    if(!hasSuccess())
        return false;
...
const std::string Fileloader::getFileName()const{
